...
describe('TtlCache', () => {
  ...
  it('does not let a load that started before invalidate() repopulate the cache', async () => {
    ...
    await expect(cache.get('k', 10_000, () => Promise.resolve('fresh'))).resolves.toBe('fresh');
  });
  ...
  it('does not join a caller that arrived after invalidate() to the load in flight', async () => {
    ...
    const afterwards = cache.get('containers:false', 10_000, () => Promise.resolve('fresh'));
    release('stale');
  ...
  it('drops only the keys it was given', async () => {
    const cache = new TtlCache();
    await cache.get('a', 10_000, () => Promise.resolve('a1'));
    await cache.get('b', 10_000, () => Promise.resolve('b1'));
    await cache.get('c', 10_000, () => Promise.resolve('c1'));

    cache.invalidate(['a', 'c']);

    await expect(cache.get('a', 10_000, () => Promise.resolve('a2'))).resolves.toBe('a2');
    await expect(cache.get('b', 10_000, () => Promise.resolve('b2'))).resolves.toBe('b1');
    await expect(cache.get('c', 10_000, () => Promise.resolve('c2'))).resolves.toBe('c2');
  });
  ...
  it('leaves an unrelated in-flight load free to populate the cache', async () => {
    ...
    await expect(cache.get('environment', 10_000, () => Promise.resolve('reloaded'))).resolves.toBe(
      'env',
    );
  });
});
